...
static struct pinmux_config bone_can_pin_mux[] = {
...
/* pinmux for chipsee backlight gpio */
static struct pinmux_config chipsee_backlight_pin_mux[] = {
...
#define AUDIO_ENABLE_GPIO GPIO_TO_PIN(3, 18)

static void chipsee_hmi_audio_init(int evm_id, int profile)
{
        setup_pin_mux(chipsee_audio_enable_pin_mux);

        gpio_request(AUDIO_ENABLE_GPIO , "");
        gpio_direction_output(AUDIO_ENABLE_GPIO, 1);

	return;
}

#define BACKLIGHT_ENABLE_GPIO GPIO_TO_PIN(1, 18)

static void chipsee_backlight_init(int evm_id, int profile)
{
        setup_pin_mux(chipsee_backlight_pin_mux);

        gpio_request(BACKLIGHT_ENABLE_GPIO, "sysfs");
        gpio_export(BACKLIGHT_ENABLE_GPIO, 1);
        gpio_direction_output(BACKLIGHT_ENABLE_GPIO, 1);
...
{
        setup_pin_mux(chipsee_buzz_pin_mux);
...
static void d_can_init(int evm_id, int profile)
{
	...
	case GEN_PURP_DDR3_EVM:
		...
		break;
	case CHIPSEE_BBBEXP:
                setup_pin_mux(bone_can_pin_mux);
                /* Instance Zero */
                am33xx_d_can_init(0);
		break;
	default:
...
static void chipsee_gpio_led_init(int evm_id, int profile)
{
        ...
        setup_pin_mux(chipsee_gpio_led_pin_mux);
        err = platform_device_register(&chipsee_leds_gpio);
        if (err)
...
static void setup_chipsee_bbbexp(void)
{
        pr_info("The board is a Chipsee Beaglebone Black Expansion.\n");

        am335x_mmc[0].gpio_wp = -EINVAL;

        _configure_device(CHIPSEE_BBBEXP, evm_chipsee_bbbexp_dev_cfg, PROFILE_NONE);
...
{
        /* Chipsee Beaglebone Black Expansion */
        setup_chipsee_bbbexp();
	return;
}
...
static struct at24_platform_data __initdata am335x_baseboard_eeprom_info = {
	.byte_len       = (256*1024) / 8,
	.page_size      = 64,
	.flags          = AT24_FLAG_ADDR16,
